...
![Home lab SIEM architecture: Kali attacks Ubuntu over SSH, Wazuh manager detects the brute force via rule 5763, and active response blocks the attacker's IP](./architecture-diagram.png)
...
![Wazuh Threat Hunting dashboard showing the full alert sequence: repeated authentication failures (rule 5760), escalation to brute-force detection (rule 5763, level 10), and the automated block (rule 651)](./wazuh-alerts-screenshot.png)
...
![Updated architecture: Kali attacks both SSH and a vulnerable web app on the Ubuntu VM. Wazuh manager detects both via built-in and custom rules. Active response reliably blocks SSH brute force; web-attack blocking is partial, a known limitation documented below.](./architecture-diagram-v2.svg)
...
An SVG version of the architecture diagram (`architecture-diagram.svg`) is also included in this repository for anyone who wants to view or edit it at full resolution.
